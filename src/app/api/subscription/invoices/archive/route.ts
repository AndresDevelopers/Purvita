...
const ArchiveInvoicesSchema = z.object({
...
export async function POST(request: NextRequest) {
  try {
    ...
    const { invoiceIds } = ArchiveInvoicesSchema.parse(payload);

    const { service } = createSubscriptionInvoiceModule(supabase);
    await service.archiveInvoices(user.id, invoiceIds);

    return NextResponse.json({
      success: true,
      message: `Archived ${invoiceIds.length} invoice${invoiceIds.length > 1 ? 's' : ''}`,
    });
  } catch (error) {
