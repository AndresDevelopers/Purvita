...
import { PayoneerPlugin } from '@/modules/payments/plugins/payoneer-plugin';
...
export async function POST(request: Request) {
    try {
        ...
        const { amount, currency, description, isTest, metadata } = CreatePayoutSchema.parse(body);

        // Fetch credentials
        const credentialsResult = await fetchGatewayCredentials<PaymentCredentials>('payoneer', isTest);
        ...
        // Initialize Plugin
        const plugin = new PayoneerPlugin();
