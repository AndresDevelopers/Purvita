...
export async function POST(request: NextRequest) {
  try {
    ...
    const { orderIds } = ArchiveOrdersSchema.parse(body);
    ...
    if (ordersError) {
      console.error('[archive-orders] Failed to fetch orders:', ordersError);
    ...
    if (updateError) {
      console.error('[archive-orders] Failed to archive orders:', updateError);
      return NextResponse.json({ error: 'Failed to archive orders' }, { status: 500 });
    }

    return NextResponse.json({
      success: true,
      message: `Successfully archived ${orderIds.length} order${orderIds.length > 1 ? 's' : ''}`,
    });
  } catch (error) {
    console.error('[archive-orders] Unexpected error:', error);
