...
interface AffiliateCartClientProps {
  lang: Locale;
  sponsor: {
    id: string;
    name: string;
...
export function AffiliateCartClient({
  lang,
  sponsor,
  dictionary: dict,
  customization,
  referralCode,
}: AffiliateCartClientProps) {
  const {
    items,
    removeItem,
    updateQuantity,
  ...
  useEffect(() => {
    const loadPhaseRewards = async () => {
      try {
        ...
        if (!session?.user?.id) {
          if (process.env.NODE_ENV !== 'production') console.log('[Affiliate Cart] No user session found');
          setPhaseReward(null);
        ...
        // ✅ SECURITY: Use fetch with timeout (10 seconds for profile data)
        const [rewardsResponse, summaryResponse, groupGainResponse] = await Promise.all([
          fetchWithTimeout('/api/profile/rewards', {
            cache: 'no-store',
            credentials: 'same-origin', // Include cookies for authentication
          }, 10000),
          fetchWithTimeout('/api/profile/summary', {
            cache: 'no-store',
            credentials: 'same-origin', // Include cookies for authentication
          }, 10000).catch((summaryError) => {
            console.error('[Affiliate Cart] Failed to request profile summary:', summaryError);
            return null;
          }),
          fetchWithTimeout('/api/products/user-discount', {
            cache: 'no-store',
          }, 10000).catch((groupGainError) => {
            console.error('[Affiliate Cart] Failed to request group gain info:', groupGainError);
            return null;
          }),
        ]);
        ...
        if (summaryResponse) {
          if (summaryResponse.ok) {
            try {
              ...
              console.log('[Affiliate Cart] Loaded membership phase:', membershipPhase);
            } catch (summaryParseError) {
              console.error('[Affiliate Cart] Failed to parse profile summary response:', summaryParseError);
            }
          } else {
            console.error(
              '[Affiliate Cart] Failed to load profile summary:',
        ...
        if (rewardsResponse.ok) {
          ...
          if (process.env.NODE_ENV !== 'production') console.log('[Affiliate Cart] Loaded phase reward:', reward);
          if (process.env.NODE_ENV !== 'production') console.log('[Affiliate Cart] User membership phase:', membershipPhase);
          ...
          console.error('[Affiliate Cart] Failed to load rewards:', rewardsResponse.status, rewardsResponse.statusText);
          setPhaseReward(null);
        }
        ...
        if (groupGainResponse && groupGainResponse.ok) {
          try {
            ...
            if (gainData?.gainPercentage > 0) {
              ...
              if (process.env.NODE_ENV !== 'production') console.log('[Affiliate Cart] Loaded phase group gain info:', gainData);
            } else {
              setPhaseGroupGain(null);
            }
          } catch (groupGainParseError) {
            console.error('[Affiliate Cart] Failed to parse group gain response:', groupGainParseError);
            setPhaseGroupGain(null);
          }
        } else {
          setPhaseGroupGain(null);
  ...
  const handleCheckout = () => {
    ...
    // Always redirect to affiliate checkout
    router.push(`/${lang}/affiliate/${referralCode}/checkout`);
  };
  ...
  const storeTitle = customization.storeTitle || `${sponsor.name}'s ${lang === 'en' ? 'Store' : 'Tienda'}`;
        ...
        <div className="max-w-4xl mx-auto">
          ...
          <div className="mb-8">
            ...
            <div className="text-sm text-muted-foreground mb-2">
              ...
              <span className="text-foreground font-medium">{dict.cart.title}</span>
            </div>

            <h2 className="text-4xl font-bold">{dict.cart.title}</h2>
          </div>
              ...
              <div className="flex items-start gap-3">
                ...
                <div className="flex-1 space-y-1">
                  <p className="font-semibold text-amber-900 dark:text-amber-100">
                    {lang === 'en'
                      ? `${sponsor.name} earns ${phaseGroupGain.gainPercentage}% from this purchase as a Phase ${phaseGroupGain.userPhase} affiliate.`
                      : `${sponsor.name} gana ${phaseGroupGain.gainPercentage}% de esta compra como afiliado Fase ${phaseGroupGain.userPhase}.`}
                  </p>
                  <p className="text-sm text-amber-700 dark:text-amber-200">
                    {lang === 'en'
                      ? 'This commission does not affect your total - it\'s our way of rewarding our affiliates.'
                      : 'Esta comisión no afecta tu total - es nuestra forma de recompensar a nuestros afiliados.'}
                  </p>
                  {groupGainAmount > 0 && (
                    <p className="text-sm font-medium text-amber-800 dark:text-amber-100">
                      {lang === 'en'
                        ? `Affiliate earnings from this cart: ${currencyFormatter.format(groupGainAmount)}`
                        : `Ganancias del afiliado de este carrito: ${currencyFormatter.format(groupGainAmount)}`}
                    </p>
                  )}
                </div>
              </div>
            ...
            <div>
              ...
              {items.length === 0 ? (
                <div className="text-center py-12">
                  <p className="text-muted-foreground">{dict.cart.emptyCart}</p>
                </div>
              ) : (
