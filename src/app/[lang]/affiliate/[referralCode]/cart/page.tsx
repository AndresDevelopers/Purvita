...
import { AffiliateCartClient } from './affiliate-cart-client';
...
/**
 * Validates affiliate and loads necessary data for cart
 */
async function getAffiliateCartData(referralCode: string) {
  try {
    ...
    console.error('[Affiliate Cart] Error loading affiliate data:', error);
    return null;
  }
}

export default async function AffiliateCartPage({ params }: Props) {
  const { referralCode, lang } = await params;
  const dict = await getLocalizedDictionary(lang);

  const affiliateData = await getAffiliateCartData(referralCode);
