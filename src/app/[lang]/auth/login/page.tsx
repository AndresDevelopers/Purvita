...
const DesktopLoginForm = ({ dict, lang, router: _router, redirectPath }: LoginFormProps) => {
  ...
  const handleLogin = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      // Call our API endpoint which has rate limiting protection
      ...
      if (!response.ok) {
        ...
        // Handle blocked account - redirect to blocked page with details
        if (response.status === 403 && data.blocked) {
          const blockedUrl = new URL(`/${lang}/auth/blocked`, window.location.origin);
          if (data.blockDetails?.reason) {
            blockedUrl.searchParams.set('reason', data.blockDetails.reason);
          }
          if (data.blockDetails?.fraudType) {
            blockedUrl.searchParams.set('type', data.blockDetails.fraudType);
          }
          if (data.blockDetails?.expiresAt) {
            blockedUrl.searchParams.set('expires', data.blockDetails.expiresAt);
          }
          window.location.href = blockedUrl.toString();
          return;
        }
      ...
      // Login successful - update Supabase session state or proceed if success flag is present
      if (data.session || data.success) {
        if (data.session) {
          const session = {
        ...
        if (data.mfaRequired) {
          ...
          const mfaRedirect = redirectPath ?? `/${lang}/dashboard`;
          window.location.href = `/${lang}/auth/mfa-verify?redirect=${encodeURIComponent(mfaRedirect)}`;
          return;
        }
        ...
        const destination = redirectPath ?? `/${lang}/dashboard`;
      ...
      setError(dict.auth?.unexpectedError ?? 'An unexpected error occurred');
    } finally {
      setLoading(false);
    }
  };
      ...
      <Card className="mx-auto w-full max-w-sm">
        ...
        <CardContent>
          ...
          <div className="mt-4 text-center text-sm">
            {dict.auth.noAccount}{' '}
            <Link href={`/${lang}/auth/register`} className="underline hover:text-primary">
...
const MobileLoginForm = ({ dict, lang, router: _router, redirectPath }: LoginFormProps) => {
  const { state, setField, setError, setLoading } = useLoginState();
  const { token: captchaToken, handleVerify, handleError: handleCaptchaError, handleExpire, executeV3, isEnabled } = useCaptcha();
    ...
    <div className="flex min-h-[100svh] flex-col justify-center gap-8 bg-background px-4 py-10">
      ...
      <form onSubmit={handleLogin} className="mx-auto flex w-full max-w-sm flex-col gap-4">
        ...
        <div className="text-center text-sm">
          {dict.auth.noAccount}{' '}
          <Link href={`/${lang}/auth/register`} className="font-semibold text-primary underline">
            {dict.navigation.register}
          </Link>
        </div>
      </form>
...
export default function LoginPage({ params }: { params: Promise<{ lang: Locale }> }) {
  ...
  const affiliateCodeParam = searchParams.get('affiliateCode');
  const affiliateSourceParam = searchParams.get('ref') ?? searchParams.get('affiliate') ?? searchParams.get('sponsor');

  // Save affiliate context when affiliateCode param is present
  useEffect(() => {
    if (affiliateCodeParam) {
      ...
      localStorage.setItem(AFFILIATE_CONTEXT_KEY, JSON.stringify(context));
    }
  }, [affiliateCodeParam]);
  ...
  const redirectPath = useMemo(() => {
    ...
    if (affiliateSourceParam) {
      return `/${lang}/affiliate/${encodeURIComponent(affiliateSourceParam)}`;
    }

    return null;
  }, [affiliateSourceParam, lang, redirectParam]);
  ...
  useEffect(() => {
    ...
    if (!supabaseUser.isLoading && supabaseUser.isAuthenticated) {
      ...
      router.replace(redirectPath ?? `/${lang}/dashboard`);
    }
  }, [hasRedirected, lang, redirectPath, router, supabaseUser.isAuthenticated, supabaseUser.isLoading]);
  ...
  if (view === 'mobile') {
    return <MobileLoginForm dict={dict} lang={lang} router={router} redirectPath={redirectPath} />;
  }

  return <DesktopLoginForm dict={dict} lang={lang} router={router} redirectPath={redirectPath} />;
}
